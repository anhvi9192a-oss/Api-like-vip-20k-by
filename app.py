...
api_keys = {
    "ducanhdev": {"exp": "30/07/2095", "remain": 99999999999, "max_remain": 9999999999, "last_reset": None}
}
...
async def get_token(acc, session):
    try:
        ...
        url = f"https://auth-garena-g171.onrender.com/token?uid={uid}&password={password}"
...
async def refresh_token():
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as s:
            gringay = await get_token("5470311526:C9ED0084096DF9C72399F6A4C43C8E77E3C1AD321E52BA675F0A37F1057F4330", s)
    except Exception:
        pass
...
def FF_LIKES():
    ...
    if is_key_valid(key) is None:
        return jsonify({"message": "key not found, To buy key contact tg @ducanhdev2"}), 200
    if not uid:
        return 'UID missing!'
    try:
        uid = str(uid).strip()
